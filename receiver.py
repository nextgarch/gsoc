...
#             # Start StreamChannel and send MasterArbitrationUpdate
#             election_id_low = random.randint(2, 999999)
#             req = p4runtime_pb2.StreamMessageRequest()
#             req.arbitration.device_id = device_id
#             req.arbitration.election_id.high = 0
#             req.arbitration.election_id.low = election_id_low
#             logger.info(f"Using election ID: {election_id_low}")

#             stream = stub.StreamChannel(iter([req]))
#             logger.info(f"Connected to P4Runtime server at {grpc_addr}")

#             # Enable digest notifications using WriteRequest
...
#             write_req.election_id.low = election_id_low
#             update = write_req.updates.add()
#             update.type = p4runtime_pb2.Update.INSERT
#             update.entity.digest_entry.CopyFrom(digest_entry)
#             stub.Write(write_req)
#             logger.info("Subscribed to digest notifications via WriteRequest")
...
#             stream = stub.StreamChannel(stream_req_iterator(req))

#             # Wait for primary confirmation
#             response = next(stream)
#             if response.HasField("arbitration"):
#                 status = response.arbitration.status
#                 if status.code != 0:
#                     raise RuntimeError(f"Not primary. Status: {status.message}")
#                 logger.info("This connection is the primary controller.")
